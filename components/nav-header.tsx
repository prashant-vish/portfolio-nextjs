...
export function NavHeader() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (windowWidth < 1024) {
        ...
        setIsNavbarVisible(window.scrollY >= 0); // Show navbar if scrolled less than 50px
      }
    };
  ...
  return (
    <div
      className={cn(
        "fixed left-0 top-0 w-full z-50 flex justify-center pt-2 lg:pt-0 transition-all duration-300"
      )}
    >
      <div
        ...
        style={{
          paddingTop:
            windowWidth >= 1024 ? `${8 + scrollProgress * 8}px` : "8px",
          paddingLeft:
            windowWidth >= 1024 ? `${scrollProgress * 16}px` : "16px",
          paddingRight:
            windowWidth >= 1024 ? `${scrollProgress * 16}px` : "16px",
        }}
      >
        <nav
          className={cn(
            "relative w-full rounded-2xl bg-black/40 backdrop-blur-xl border border-white/10 lg:transition-all lg:duration-300 transition-opacity duration-300 ease-in-out transform",
            {
              "opacity-0 translate-y-[-100%]":
                !isNavbarVisible && windowWidth < 1024, // Hide navbar with transition
              "opacity-100 translate-y-0":
                isNavbarVisible && windowWidth < 1024, // Show navbar
            }
          )}
          style={{
        ...
        >
          <div className="h-16 flex items-center justify-between px-6">
            ...
            <div className="hidden lg:flex items-center gap-2">
              <div className="flex items-center gap-1">
                <Link
                  href="https://leetcode.com/u/prashant_vish/"
                  target="_blank"
            ...
            <button
              ...
              className={cn(
                "lg:hidden p-2 hover:bg-white/5 rounded-lg transition-colors",
          ...
          {mobileMenuOpen && (
            <div
              ref={mobileMenuRef}
              className="absolute left-0 right-0 top-14 z-50 lg:hidden border-t border-white/10 bg-black/90 backdrop-blur-xl rounded-b-2xl shadow-lg"
            >
